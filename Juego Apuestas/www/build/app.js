...
angular.module('App', ['ionic','firebase','App.controladores', 'App.servicioFirebase','ngCordova', 'ngAnimate'])
...
.state('app.Piano', {
	...
	.state('app.Ola', {
            ...
            views: {
                viewContent: {
	            templateUrl: "templates/ola.html",
	            controller: 'contolerOla'
	        }}})
...
console.log("POR AHORA creo bd dinamica para mostrar el rankig de usuarios.");
window.queries = [
	//Drop tables
   "DROP TABLE IF EXISTS Users;",
	//Create tables
	"CREATE TABLE Users (IdUser integer primary key autoincrement, apodo text not null);",
	//Insert Users
	"INSERT INTO 'Users' ('apodo') VALUES ('POR AHORA creo bd dinamica para mostrar el rankig de usuarios');",
	"INSERT INTO 'Users' ('apodo') VALUES ('La idea es q aca sea una lista con los user  y su puntaje rankeados');",
	"INSERT INTO 'Users' ('apodo') VALUES ('Matias Ds');",
	"INSERT INTO 'Users' ('apodo') VALUES ('Octavio Vill');",
	"INSERT INTO 'Users' ('apodo') VALUES ('Etc 1');",
	"INSERT INTO 'Users' ('apodo') VALUES ('Etc 3');"
];
(function () {
	'use strict';
        ...
        $scope.abrirMenuIzq = function ($event) {
           ...
           $scope.openItem = function(item){
            try{				// le saco esta parte si le saco los juegos del menu
    				$state.go("app."+item.title);
    				}    
					catch(e) {$state.go(item.title);}
...
(function () {
	...
	function $sqliteService($q, $cordovaSQLite) {
		...
		self.preloadDataBase = function (enableLog) {
			...
			if (window.sqlitePlugin === undefined) {
				enableLog && console.log('%c ***************** creacion de la bd en el explorer ***************** ', 'background: #222; color: #bada55');
				self.db().transaction(function (tx) {
					...
					enableLog && console.log('%c ***************** creacion de la bd en el explorer completa***************** ', 'background: #222; color: #bada55');
					deferred.resolve("OK");
				});
			}
			else {
...
(function () {
	...
	function Users($q, $sqliteService) {
		...
		return {
			...
			add: function (user) {
				var query = "INSERT INTO Users (apodo) VALUES (?)";
				return $q.when($sqliteService.executeSql(query, [user.apodo]));
			}
		};
	}
})();
